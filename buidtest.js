...
const SETTINGS_KEY = 'queenty_settings';
...
async function init() {
  ...
  // 1. Attente active que Firebase Auth soit chargé et actif
  const authPret = await attendreAuthFirebase();
  if (authPret) {
    window.authModules.onAuthStateChanged(window.auth, async (user) => {
      if (user) {
        currentUid = user.uid; // Récupération sécurisée du profil utilisateur
        
        // Lancement des initialisations dépendantes de l'utilisateur
        loadIssuerSettings();
        setupLogoUpload();
...
// Petite fonction pour afficher des alertes visuelles modernes (sans utiliser alert() natif)
...
async function saveInvoice() {
  ...
  if (items.length === 0) {
    afficherNotification('Ajoute au moins un article avant de sauvegarder.', 'error');
    return;
  }

  const clientName = getValue('client-name-input');
  if (!clientName) {
    afficherNotification('Le nom du client est requis.', 'error');
    return;
  }
  ...
  if (fb && currentUid) {
    try {
      ...
      // Utilisation du chemin sécurisé par UID
      const facturesColRef = collection(db, 'users', currentUid, 'factures');
      const querySnapshot = await getDocs(facturesColRef);
      ...
      if (existingDocId) {
        await updateDoc(doc(db, 'users', currentUid, 'factures', existingDocId), invoiceData);
      } else {
        await addDoc(facturesColRef, invoiceData);
      }

      hideLoading();
      afficherNotification('Facture sauvegardée avec succès !');
      setTimeout(() => { window.location.href = 'dashboard.html'; }, 1000);

    } catch (err) {
      console.error("Erreur d'enregistrement Firebase :", err);
      hideLoading();
      afficherNotification("Erreur lors de la sauvegarde sur Firebase.", "error");
    }
  }
}
