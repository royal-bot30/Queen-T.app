 ...
 * Stockage : Firebase Firestore (collections isolées par utilisateur : users/{uid}/produits et users/{uid}/semaines).
 * Nécessite que le script de configuration Firebase (window.db + window.dbModules + window.auth)
 * soit chargé AVANT ce fichier dans le HTML.
 */

const StockApp = {
    uid: null, // L'identifiant unique de l'utilisateur
    produits: [],
    ...
    async init() {
        this.bindEvents(); // On attache les boutons UI immédiatement
        
        // Attente active que window.auth soit injecté par Firebase (max 5 secondes)
        const authPret = await this.attendreAuthFirebase();
        ...
        if (authPret) {
            window.authModules.onAuthStateChanged(window.auth, async (user) => {
                if (user) {
                    ...
                    console.warn("Aucun utilisateur actuellement connecté.");
                }
            });
        } else {
            this.afficherErreur("Firebase Auth n'a pas pu être initialisé (vérifie tes scripts HTML).");
        }
    },

    // Petite fonction utilitaire pour patienter pendant le chargement du module Firebase
    attendreAuthFirebase() {
        return new Promise((resolve) => {
            ...
            const interval = setInterval(() => {
                ...
                if (window.auth && window.authModules) {
                    ...
                    resolve(true);
                } else if (tentatives > 50) { // S'arrête après 5 secondes si non trouvé
                    clearInterval(interval);
                    resolve(false);
                }
            }, 100);
    ...
    getFirebase() {
        if (!window.db || !window.dbModules) {
            this.afficherErreur("Firebase n'est pas initialisé. Vérifie que le script de configuration est bien chargé.");
            return null;
        }
        return { db: window.db, ...window.dbModules };
    },

    // Nouvelle fonction pour centraliser les chemins isolés par utilisateur
    getRefs() {
        const fb = this.getFirebase();
        if (!fb) return null;
        if (!this.uid) {
            this.afficherErreur("Utilisateur non identifié pour accéder aux données.");
            return null;
        }
        const { db, collection, doc } = fb;
        return {
            produitsCol: collection(db, 'users', this.uid, 'produits'),
            semainesCol: collection(db, 'users', this.uid, 'semaines'),
            produitDoc: (id) => doc(db, 'users', this.uid, 'produits', id),
            semaineDoc: (id) => doc(db, 'users', this.uid, 'semaines', id),
            ...fb // On retourne aussi addDoc, updateDoc, deleteDoc, getDocs, etc.
        };
    },
    ...
    async chargerDonnees() {
        ...
        if (!refs) return;
        const { getDocs, produitsCol, semainesCol } = refs;

        try {
            const snapProduits = await getDocs(produitsCol);
            this.produits = snapProduits.docs
                .map(d => ({ id: d.id, ...d.data() }))
                .filter(p => p.actif !== false);

            const snapSemaines = await getDocs(semainesCol);
    ...
    async ajouterProduit(nom, prix) {
        const refs = this.getRefs();
        if (!refs) return;
        const { addDoc, produitsCol } = refs;

        try {
            const docRef = await addDoc(produitsCol, { nom, prix, actif: true });
            ...
            this.afficherErreur("Erreur lors de l'ajout du produit.");
            console.error(err);
        }
    },
    ...
    async mettreAJourProduitBDD(id, nom, prix) {
        const refs = this.getRefs();
        if (!refs) return;
        const { updateDoc, produitDoc } = refs;

        try {
            await updateDoc(produitDoc(id), { nom, prix });
    ...
    async supprimerProduit(id, nomProduit) {
        const confirme = await new Promise(resolve => {
            this.afficherModaleConfirmation(`Voulez-vous vraiment supprimer "${nomProduit}" ?`, resolve);
        });
        if (!confirme) return;
        ...
        try {
            await updateDoc(produitDoc(id), { actif: false });
    ...
    async sauvegarderSemaineActuelle() {
        ...
        const refs = this.getRefs();
        if (!refs) return;
        const { updateDoc, semaineDoc } = refs;

        try {
            await updateDoc(semaineDoc(this.semaineActuelleId), {
    ...
    /* ---------------- Gestion Historique (Modifier / Supprimer) ---------------- */

    async supprimerSemaineHistorique(semaineId, numero) {
        const confirme = await new Promise(resolve => {
            this.afficherModaleConfirmation(`Supprimer définitivement la Semaine #${numero} ?`, resolve);
        });
        if (!confirme) return;

        const refs = this.getRefs();
        if (!refs) return;
        const { deleteDoc, semaineDoc } = refs;

        try {
            await deleteDoc(semaineDoc(semaineId));
    ...
    modifierSemaineHistorique(semaineId) {
        ...
        box.querySelector('#sauver-edit-sem').onclick = async () => {
            ...
            if (refs) {
                try {
                    const { updateDoc, semaineDoc } = refs;
                    await updateDoc(semaineDoc(semaineId), { lignes: semaine.lignes });
                } catch (err) {
    ...
    demarrerNouvelleSemaine() {
        const message = this.semaineActuelle
            ? `Tu vas commencer une nouvelle semaine. La semaine #${this.semaineActuelle.numero} sera archivée. Continuer ?`
    ...
    async _creerNouvelleSemaine() {
        const refs = this.getRefs();
        if (!refs) return;
        const { addDoc, updateDoc, semainesCol, semaineDoc } = refs;

        try {
            if (this.semaineActuelleId) {
                await updateDoc(semaineDoc(this.semaineActuelleId), { active: false });
                if (this.semaineActuelle) {
            ...
            const numeroSemaineActuel = (this.historiqueSemaines.length > 0) ? this.historiqueSemaines[0].numero + 1 : (this.semaineActuelle ? this.semaineActuelle.numero + 1 : 1);
            ...
            const docRef = await addDoc(semainesCol, nouvelleSemaine);
            this.semaineActuelle = { id: docRef.id, ...nouvelleSemaine };
            this.semaineActuelleId = docRef.id;

            this.rafraichirInterface();
        } catch (err) {
            this.afficherErreur('Erreur lors de la création de la nouvelle semaine.');
            console.error(err);
        }
    },
    ...
    afficherModaleConfirmation(message, callback) {
        ...
        const box = document.createElement('div');
        box.style.cssText = 'background:#fff;padding:24px;border-radius:10px;max-width:340px;width:100%;text-align:center;box-shadow:0 10px 30px rgba(0,0,0,0.2);';
        ...
        btnAnnuler.style.cssText = 'padding:10px 18px;border-radius:6px;border:1px solid #ccc;background:#f5f5f5;cursor:pointer;font-size:14px;';

        const btnConfirmer = document.createElement('button');
        btnConfirmer.type = 'button';
        btnConfirmer.textContent = 'Confirmer';
        btnConfirmer.style.cssText = 'padding:10px 18px;border-radius:6px;border:none;background:#ff007f;color:#fff;cursor:pointer;font-size:14px;';
    ...
    /* ---------------- Rendu de l'interface & Filtrage des résumés ---------------- */
    ...
    rendreBlocSemaine() {
        ...
        if (this.semaineActuelle) {
            ...
            if (numSemaineEl) numSemaineEl.textContent = 'Aucune Semaine';
    ...
    rendreTableauSuivi() {
        ...
        if (!this.semaineActuelle || this.semaineActuelle.lignes.length === 0) {
            tbody.innerHTML = `<tr id="message_aucune_semaine"><td colspan="6">Cliquez sur "Nouvelle semaine" pour commencer.</td></tr>`;
            return;
        }
    ...
    calculerEtAfficherResumes() {
        ...
        const filtreActif = (this.filtreSemaines.min !== null || this.filtreSemaines.max !== null);

        if (filtreActif) {
            const semainesFiltrees = this.getSemainesFiltrees();
            semainesFiltrees.forEach(sem => {
                (sem.lignes || []).forEach(l => {
                    totalRestant += l.restant || 0;
                    totalVendu += l.vendu || 0;
                    chiffreAffairesVentes += (l.vendu || 0) * (l.prix || 0);
                });
            });
    ...
    actualiserGraphique() {
        ...
        const semaines = this.getToutesLesSemaines();
    ...
    afficherHistorique() {
        ...
        const semaines = [...this.historiqueSemaines].sort((a, b) => b.numero - a.numero);

        if (semaines.length === 0) {
            container.innerHTML = `<p style="padding: 10px 0;">Aucune semaine archivée pour le moment.</p>`;
            return;
        }
                ...
                <div class="historique-semaine-item" style="border-bottom:1px solid #ddd;padding:10px 0;">
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
                        ...
                        <div style="display:flex;gap:5px;">
                            <button type="button" style="background:none;border:none;cursor:pointer;font-size:16px;" onclick="StockApp.modifierSemaineHistorique('${sem.id}')" title="Modifier">✏️</button>
                            <button type="button" style="background:none;border:none;cursor:pointer;font-size:16px;" onclick="StockApp.supprimerSemaineHistorique('${sem.id}', ${sem.numero})" title="Supprimer">🗑️</button>
                    ...
                    <div class="historique-semaine-detail" style="display:none;margin-top:10px;overflow-x:auto;">
                        <table class="data-table" style="width:100%;font-size:13px;">
                            ...
                            <tbody>${lignesHtml || '<tr><td colspan="4">Aucun produit.</td></tr>'}</tbody>
                        </table>
                    </div>
                </div>
            `;
